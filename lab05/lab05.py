...
class LinkedList:
    ...
    def insert(self, idx, value):
        ...
        for i in range(idx):
            cur = cur.next
        cur.prior = LinkedList.Node(value, cur.prior, cur)
    ...
    def __eq__(self, other):
        ...
        ### BEGIN SOLUTION
        if type(self) != type(other) or self.length != other.length:
            return False
        cur1 = self.head
        cur2 = other.head
        for i in range(self.length):
