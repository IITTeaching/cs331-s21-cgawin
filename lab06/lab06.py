...
def check_delimiters(expr):
    ...
    for d in expr:
        if d in delim_openers:
            stack.push(d)
        elif d in delim_closers:
            if stack.empty():
                return False
            if delim_openers.index(stack.peek()) == delim_closers.index(d):
                stack.pop()
            else:
                return False
    ...
    def resize(self, newsize):
        ...
        for j in self:
            if j is not None:
                new[i] = j
                i += 1
        new = [None] * (newsize - len(self.data)) + new
        self.head, self.tail, self.data = -1, len(self.data), new
        ### END SOLUTION
